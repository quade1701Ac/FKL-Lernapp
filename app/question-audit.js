...
function tokens(s=''){return new Set(norm(s).split(' ').filter(x=>(x.length>3||/^\d+$/.test(x))&&!STOP.has(x)))}
function similarity(a,b){const A=tokens(a),B=tokens(b);if(!A.size||!B.size)return 0;let hit=0;A.forEach(x=>{if(B.has(x))hit++});return hit/(A.size+B.size-hit)}
function validShape(q){if(!q||!q.id||!q.question||!Number.isInteger(q.field)||q.field<1||q.field>13)return false;if(!['free','mc','number','order'].includes(q.type))return false;if(q.type==='mc'){if(!Array.isArray(q.options)||q.options.length<3||!Array.isArray(q.correct)||!q.correct.length||q.correct.some(i=>!Number.isInteger(i)||i<0||i>=q.options.length)||new Set(q.correct).size!==q.correct.length)return false;const opts=q.options.map(norm);if(new Set(opts).size!==opts.length)return false}if(q.type==='order'&&(!Array.isArray(q.items)||q.items.length<3||new Set(q.items.map(norm)).size!==q.items.length))return false;if(q.type==='number'&&(typeof q.answer!=='number'||!Number.isFinite(q.answer)||typeof q.tolerance!=='number'||!Number.isFinite(q.tolerance)||q.tolerance<0))return false;if(q.type==='free'&&(!Array.isArray(q.keywords)||!q.keywords.length||!q.solution))return false;return true}
