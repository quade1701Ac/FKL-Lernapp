...
function tooEasy(q){if(q.type!=='mc')return false;const text=norm(q.question),d=Number(q.difficulty)||1;if(d<=1&&/wofuer steht|was bedeutet|welches beispiel|was ist ein|welcher vorteil|welche kontrolle|wozu dient|was bezeichnet|welche information/.test(text))return true;const absurd=(q.options||[]).filter(o=>/lieblingsfarbe|lohnsteuer|urlaubsplanung|bilanzsumme|radioleistung|sitzbezug|arbeitsvertrag|pausenraum|automatisch billiger|verkaufspreis/.test(norm(o))).length;return d<=1&&absurd>=2}
